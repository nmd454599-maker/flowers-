...
android {
    namespace = "com.azharna.azharna_pro"
    ...
    defaultConfig {
        ...
        applicationId = "com.azharna.azharna_pro"
        if (previewBuild) applicationIdSuffix = ".preview"
        manifestPlaceholders["appLabel"] = if (previewBuild) "أزهارنا — معاينة" else "أزهارنا"
...
// The independent visual preview does not initialize Firebase.
if (previewBuild) {
    tasks.configureEach {
        if (name.endsWith("GoogleServices")) enabled = false
    }
}
